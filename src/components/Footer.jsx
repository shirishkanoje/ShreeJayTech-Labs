...
const Footer = () => {
  ...
  const handleClick = (url) => {
    ...
    // External (WhatsApp, Form, Email)
    if (url.startsWith("http") || url.startsWith("mailto")) {
      window.open(url, "_blank")
    }

    // Internal route (/verify)
    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
            ...
            <div key={link.title} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className='list-none mt-4'>
                {link.links.map((item, index) => (
                  <li 
                    key={item.name} 
                    onClick={() => handleClick(item.link)}   // 🔥 FIX HERE
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
        ...
        <p className='font-poppins font-normal text-center text-[18px] leading-[27px] text-white'>
          2025 ShreeTech Labs. All Rights Reserved.
        </p>  
        ...
        <div className='flex flex-row md:mt-0 mt-6'>
          {socialMedia.map((social, index) => (
            <img
              ...
              key={social.id}
              alt={social.id}
              onClick={() => handleClick(social.link)}   // 🔥 FIX HERE
